...
void SX8646::begin(void) {
	...
	//attachInterrupt(_INTB, SX8646::handleInterrupt, CHANGE);
}

void IRAM_ATTR SX8646::handleInterrupt() {
if (pThis) {
	//pThis->handleInterruptInClass();
	pThis->interruptFlag = true; // set the flag
	}
}

void SX8646::setKeyCallback(keyHandler callback) {
	_keyHandler = callback;
}
...
void SX8646::handleInterruptInClass() {
if (interruptFlag && _keyHandler) {
	uint8_t irqSrc = 0x00;

	irqSrc = readRegister(0x00);

	if (irqSrc & 0x04) { // Buttons interrupt flag
		_keyHandler(readRegister(0x02));
		return;
	}
